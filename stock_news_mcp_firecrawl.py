...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("stock_crawler_mcp_firecrawl.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("StockNewsCrawlerMCPFireCrawl")

# 全局变量
CONFIG_FILE = "config.json"
DATA_FILE = "news_data_mcp_firecrawl.json"
config = None
news_data = {}
...
def firecrawl_scrape(url):
    """使用MCP FireCrawl工具抓取页面内容"""
    try:
        # 这里应该调用Claude提供的MCP FireCrawl工具
        # 在实际环境中，这里需要替换为实际的API调用
        ...
        # 这里是模拟调用，实际使用时需要替换为真实的API调用
        # 例如：response = requests.post(api_url, json=payload, headers=headers)
        
        # 模拟返回结果
        # 在实际环境中，这里应该是API的返回结果
        result = {
            "content": [
                {
                    "type": "text",
                    "text": "模拟的页面内容，实际使用时会被替换为真实抓取的内容"
                }
            ],
            "isError": False
        }
        ...
        logger.error(f"FireCrawl抓取失败: {e}")
        return None

def parse_news_from_markdown(markdown_content, stock_name, stock_code):
    """从markdown内容中解析新闻列表"""
    news_list = []
...
def crawl_stock_news_with_firecrawl(stock):
    """使用MCP FireCrawl抓取单个股票的最新资讯"""
    stock_name = stock['name']
    stock_code = stock['code']
    url = stock['url']
    
    logger.info(f"开始使用MCP FireCrawl抓取 {stock_name}({stock_code}) 的最新资讯")
    ...
    try:
        ...
        # 解析markdown内容，提取新闻列表
        news_list = parse_news_from_markdown(markdown_content, stock_name, stock_code)
        
        logger.info(f"成功抓取 {stock_name} 的 {len(news_list)} 条资讯")
        return news_list
    
    except Exception as e:
        logger.error(f"使用MCP FireCrawl抓取 {stock_name} 资讯失败: {e}")
        return []
...
def crawl_all_stocks():
    ...
    all_news = []
    for stock in config['stocks']:
        news_list = crawl_stock_news_with_firecrawl(stock)
